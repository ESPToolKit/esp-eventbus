...
template <typename T> class EventBusAllocator {
  public:
	using value_type = T;
	...
	template <typename U>
	EventBusAllocator(const EventBusAllocator<U> &other) noexcept
	...
	template <typename U> bool operator==(const EventBusAllocator<U> &other) const noexcept {
  ...
  private:
	template <typename> friend class EventBusAllocator;

	bool usePSRAMBuffers_ = false;
};

template <typename T> using EventBusVector = std::vector<T, EventBusAllocator<T>>;
