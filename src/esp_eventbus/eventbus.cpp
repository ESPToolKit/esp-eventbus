...
bool ESPEventBus::init(const EventBusConfig &config) {
	...
	running_ = true;
	const char *taskName =
	    (config_.taskName && config_.taskName[0] != '\0') ? config_.taskName : "ESPEventBus";
...
EventBusSub ESPEventBus::subscribe(EventBusId id, EventCallbackFn cb, void *userArg, bool oneshot) {
	if (!cb) {
		return 0;
	}
...
EventBusSub ESPEventBus::subscribe(EventBusId id, EventCallback cb, void *userArg, bool oneshot) {
	if (!cb || !subMutex_) {
		return 0;
	}
	...
	if (config_.maxSubscriptions != 0) {
		size_t activeCount = std::count_if(subs_.begin(), subs_.end(), [](const Subscription &sub) {
			return sub.active;
		});
...
void ESPEventBus::taskLoop() {
...

	{
		...
		EventBusVector<Subscription> snapshot{
		    EventBusAllocator<Subscription>(config_.usePSRAMBuffers)
		};
		snapshot.reserve(4);
...
void ESPEventBus::emitPressureMetricFromTask() {
	if (!config_.pressureCallback || !queue_ || config_.queueLength == 0 ||
	    config_.pressureThresholdPercent == 0) {
		return;
	}
...
void ESPEventBus::propagateYieldFromISR(
    BaseType_t localWoken, BaseType_t *higherPriorityTaskWoken
) {
...
bool ESPEventBus::createKernelMutex() {
	...
	if (config_.usePSRAMBuffers) {
		...
		if (mutexStorage_) {
			subMutex_ =
			    xSemaphoreCreateMutexStatic(static_cast<StaticSemaphore_t *>(mutexStorage_));
			if (subMutex_) {
				return true;
			}
...
#if defined(configSUPPORT_STATIC_ALLOCATION) && (configSUPPORT_STATIC_ALLOCATION == 1)
	if (config_.usePSRAMBuffers) {
		queueStorage_ = allocateKernelStorage(
		    static_cast<size_t>(config_.queueLength) * sizeof(QueuedEvent),
		    true
		);
...
ESPEventBus::WaiterContext *
ESPEventBus::findWaiterLocked(TaskHandle_t ownerTask, EventBusId eventId) {
	for (auto &waiter : waiters_) {
